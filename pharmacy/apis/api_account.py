...
from pharmacy.models import NewCompanyName, NewItemName, NewNumberOfItem, NewPlaceName, NewPrice, NewSize,  NewSourceName ,ShoppingBill,Governorate,Zone,CustomerLocation,CustomerInformation,Item
    ...
    for shoppingBill in shoppingBills:
        ...
        if shoppingBill.delivered:
            for item in shoppingBill.item.select_related(
                ...
                totalAccountPrice+=(item.numberOfItem.number*item.itemPrice.price)     
        if not shoppingBill.delivered:
            for item in shoppingBill.item.select_related(
                ...
                #totalItemNumber+=item.numberOfItem
                totalAccountPrice+=(item.numberOfItem.number*item.itemPrice.price)
                generalOutcome+=(item.numberOfItem.number*item.sourcePrice.price)
        ...
        result.append({
            ...
            'place': f'{shoppingBill.location.governorate.name} {shoppingBill.location.zone}',
            # 'itemsnumber':totalItemNumber,
...
def get_account(request,id:str):
    shoppingBill=ShoppingBill.objects.select_related('customerInformation',
                                                    'location',
                                                    'location__governorate',
                                                    'location__zone',
                                                    ).get(billNumber=id)
    ...
    for item in shoppingBill.item.select_related(
        ...
        salesitems+=[{
            ...
            'unitPrice':item.sourcePrice.price,
            'numberOfItem':item.numberOfItem.number,
            'ItemPrice':item.itemPrice.price,
        }]
        totalAccountPrice+=(item.numberOfItem.number*item.itemPrice.price)   
    if shoppingBill.delivary:
